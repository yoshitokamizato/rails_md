...
Gemfileの編集が終わったら`bundle install`を実行し、gemをインストールします。

```
bundle install
```

`application.css`の拡張子を`scss`に変更します。
...
<!-- `application.scss`の`*= require_tree .`を削除します。このコードは、`app/assets/stylesheets/`配下のファイルを全て読み込むという設定をします。これにより、意図しないファイルを読み込まれデザインが崩れたりするので、`Bootstrap`を実装するしないに関わらず、削除したほうがいいです。
...
レスポンシブに対応させるため、以下のコードを application.html.erbのheadタグの中に記述します。

```
<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
```

以下のコードをhtml.erbに記述し、Bootstrapが適用されているか確かめます。
...
青色のボタンが表示されていれば、Bootstrapの実装は完了です。他にもたくさん利用できるパーツがあるので、色々と試してみてください。
